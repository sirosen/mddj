...
def test_read_string_table_key(tmp_path):
    ...
    read_val = _readers.read_pyproject_toml_value(pyproject, "project", "version")
...
def test_read_array_members(tmp_path):
    ...
    read_val = _readers.read_pyproject_toml_value(pyproject, "mytable", "items", 0)
    assert read_val == "foo"
...
def test_read_bad_lookup_noncontainer(tmp_path):
    ...
    with pytest.raises(LookupError, match="Terminated in a non-container type"):
        _readers.read_pyproject_toml_value(pyproject, "mytable", "foo", "bar")
