...
from .system_info import SystemInfoReader, _SystemInfoReaderImplementation
from .tox import ToxReader, _ToxReaderImplementation
...
class Reader(t.Protocol):
    ...
    def name(self) -> str:
        ...
        if value is None:
            raise _errors.MissingRequiredField(
...
def _requires_python_lower_bound(req: str) -> str:
    ...
    for r in reqs:
        if r.startswith(">="):
            ...
            raise _errors.NotALowerBound(
                "Found a > requirement, rejecting as invalid (use '>=' instead)"
            )
    if len(lower_bounds) > 1:
        raise _errors.NotALowerBound("Found multiple lower bounds, cannot choose one")
    elif len(lower_bounds) == 0:
        raise _errors.NotALowerBound("Found no lower bounds")
    else:
